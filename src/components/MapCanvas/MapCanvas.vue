<template>
  <div>
    ...
    <img src="./img/timg.jpg" @load="draw" alt id="img-canvas" />
  </div>
</template>
<script>
export default {
  ...
  computed: {
    ...
    direction() {
      let DeltaX =
        this.getMaxLongitude(this.coordinateList) -
        this.getMinLongitude(this.coordinateList);
      let DeltaY =
        this.getMaxLatitude(this.coordinateList) -
        this.getMinLatitude(this.coordinateList);
      return DeltaX / DeltaY > this.width / this.height
        ? "horizontal"
        : "vertical";
    },
    coordinateListToCanvas() {
      return this.coordinateList.map(item =>
        this.computedGeographicCoordinatesToCanvasCoordinates(item)
      );
    }
  },
  methods: {
    ...
    computedCanvasCoordinatesToGeographicCoordinates(coordinate) {
      let coordinateToCenter = [
        coordinate[0] - this.width / 2,
        coordinate[1] - this.height / 2
      ];
      let coefficient = 0;
      ...
      return [
        coordinateToCenter[0] * coefficient + center.longitude,
        -coordinateToCenter[1] * coefficient + center.latitude
      ];
    }
  }
};
